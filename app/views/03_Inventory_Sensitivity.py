...
st.markdown(
    ...
    'remain a winner under another.</p>',
...
# ---------- controls beside the chart (DESIGN.md §12) ----------
...
with main_col:
    ...
    w = cell.iloc[0]
    st.markdown(
        f'<div class="insight"><div class="h">Cheapest at L={lt} · service {sv:.0%} · P={px}</div>'
        f'<div class="w"><b>{w["model"]}</b> at cost <b>{w["total_cost"]:.2f}</b>'
        + (" — reproduces the published frozen claim." if is_frozen
           else " — an interactive scenario, not the published result.") + "</div></div>",
        unsafe_allow_html=True,
    )
...
kpi_strip([
    ...
    ("Avg inventory", f"{w['avg_inventory']:.2f}", "units"),
])
fig2 = fig_base(height=400)
...
with base:
    ...
    fig3.update_layout(
        title=dict(text=f"Cost change: {DS[ds]} L{clt}·{csv:.0%}·P{cpx} − L{lt}·{sv:.0%}·P{px} · green = scenario cheaper",
                   font=dict(size=15, color=WHITE), x=.01),
        xaxis_title="Δ total cost")
    chart_panel(fig3, "", "Read: moving right on this chart means the compared policy is more expensive for that model.")

# ---------- policy robustness ----------
st.subheader("Who wins across all 27 policies?")
